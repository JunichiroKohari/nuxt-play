...
export default {
  ...
  methods: {
    ...
    openDocument () {
      this.buildingDocument = !this.buildingDocument
      this.src.promise.then(pdf => {
        const test = document.getElementById('pdf')
        console.log(test)
        const canvas = document.getElementById('pdf').childNodes[0]
        const context = canvas.getContext('2d')
        
        test.addEventListener('click', (e) => {
          const absolutePosition = e.target.getBoundingClientRect()
          let x = e.clientX - absolutePosition.left
          let y = e.clientY - absolutePosition.top
          console.log(x)
          console.log(y)
        })
        console.log(canvas)
